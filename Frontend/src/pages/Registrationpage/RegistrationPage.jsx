...
export default function RegistrationPage({ onSuccess }) {
  ...
  const [formData, setFormData]             = useState({ fullName:'', mobile:'', email:'', fieldOfStudy:'', yearOfStudy:'', demoSlot:'' });
          ...
          <form onSubmit={handleSubmit} className="rp-form" noValidate>
            ...
            <div className="rp-row-2">
              ...
              <div className="fg">
                ...
                <div
                  className={`slot-trigger-btn ${formData.demoSlot ? 'has-slot' : ''} ${errors.demoSlot && touched.demoSlot ? 'has-error' : ''}`}
                  onClick={() => setShowSlotPopup(true)}
                >
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                  <span className={formData.demoSlot ? 'slot-filled-text' : 'slot-placeholder-text'}>
                    {formData.demoSlot || 'Pick date & time…'}
                  </span>
                  {formData.demoSlot
                    ? <button type="button" className="slot-clear-inline" onClick={e => { e.stopPropagation(); setFormData(p=>({...p,demoSlot:''})); setTouched(p=>({...p,demoSlot:false})); }}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                      </button>
                    : <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{width:13,height:13,opacity:.5}}><polyline points="6 9 12 15 18 9"/></svg>
                  }
                </div>
